...
> **▶ NÄCHSTER LAUF (ab 2026-06-27):** **(A) Wesel ERLEDIGT 2026-06-25** (`dda4aff`). **(B) FAQPage index.html ERLEDIGT 2026-06-26** (Commit `309f38a`, 2 JSON-LD-Blöcke, FAQPage 5 Fragen, live verifiziert). **NÄCHSTES:** Doorway-Differenzierung mit `seo-content` fortsetzen — nächsten Kandidaten per 5-Gramm-Jaccard wählen (offen: recklinghausen, dinslaken, dorsten, duisburg, herne, oberhausen, bottrop, haltern). Optional: FAQPage-Methode auf Service-Seiten ausrollen (inhaltsneutral, keine Freigabe nötig). Offen GSC: Indexierung für `zauberer-wesel.html` + `/` (manuell durch Inhaber, Deep-Link liefert 404). Nach jeder Änderung Standard-Verifikation.
...
- [x] **FAQPage-Schema auf index.html — ERLEDIGT 2026-06-26 (Commit `309f38a`).** FAQPage-JSON-LD aus den 5 Bestands-Q&As byte-genau als ZWEITER separater `<script>`-Block nach LocalBusiness vor `</head>` ergänzt (kein @graph). Live verifiziert: HTTP 200, 26.799 B, 2 JSON-LD-Blöcke, FAQPage parst mit 5 Fragen, 0 U+FFFD. Übertragbar auf weitere FAQ-Seiten (Service-Seiten) — reine Schema-Pflege.
